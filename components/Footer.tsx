        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-8 mb-12">
          ...
          <div className="col-span-1 sm:col-span-2 md:col-span-4 lg:col-span-2">
            ...
            <p className="text-secondary-300 mb-6 max-w-sm">
              {companyInfo?.tagline || 'Empowering enterprises with cutting-edge IT integration and technology solutions for digital transformation and sustainable growth.'}
            </p>
            <div className="flex space-x-4">
              {companyInfo && [
                { name: 'Facebook', href: companyInfo.social.facebook, key: 'facebook' },
                { name: 'Twitter', href: companyInfo.social.twitter, key: 'twitter' },
                { name: 'LinkedIn', href: companyInfo.social.linkedin, key: 'linkedin' },
                { name: 'Instagram', href: companyInfo.social.instagram, key: 'instagram' },
              ].map((social) => (
                <a
                  key={social.name}
                  href={social.href}
                  className="w-10 h-10 rounded-full bg-secondary-800 flex items-center justify-center hover:bg-accent transition-colors duration-300"
                  aria-label={social.name}
                  target="_blank"
                  rel="noopener noreferrer"
                >
                  <svg className="w-5 h-5 fill-current" viewBox="0 0 24 24">
                    <path d={socialIconPaths[social.key]} />
                  </svg>
                </a>
              ))}
            </div>
